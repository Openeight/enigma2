		...
		if current:
			...
			if self.menu == 0:
				if currentEntry == 'XTDVBNTPTimecheck':
					if path.exists("/usr/lib/enigma2/python/Plugins/Extensions/ExtrasPanel/dvbdate"):
						self.session.open(Console, _('Checking DVB Time...'), ['/usr/lib/enigma2/python/Plugins/Extensions/ExtrasPanel/dvbdate --print'])
					else:
						self.session.open(MessageBox, _('Sorry,\nDVB Time Check is not available for this box at the moment.'), MessageBox.TYPE_INFO, timeout=10)
