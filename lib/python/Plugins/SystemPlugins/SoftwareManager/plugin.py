...
from enigma import eTimer, RT_HALIGN_LEFT, RT_VALIGN_CENTER, eListbox, gFont, getDesktop, ePicLoad, eRCInput, getPrevAsciiCode, eEnv, getBoxType, getEnigmaVersionString
...
config.plugins.configurationbackup.backupdirs = ConfigLocations(default=[eEnv.resolve('${sysconfdir}/enigma2/'), '/etc/CCcam.cfg', '/usr/keys/',
																		'/usr/bin/*cam*', '/etc/init.d/softcam*',
																		'/etc/tuxbox/config/', '/etc/*.emu', '/etc/auto.network', '/etc/enigma2/automounts.xml',
																		'/etc/MultiQuickButton/',
																		'/etc/samba/', '/etc/fstab', '/etc/inadyn.conf',
																		'/etc/network/interfaces', '/etc/wpa_supplicant.conf', '/etc/wpa_supplicant.ath0.conf',
																		'/etc/wpa_supplicant.wlan0.conf', '/etc/resolv.conf', '/etc/default_gw', '/etc/hostname', '/etc/opkg/secret-feed.conf',
			...
			if self.menu == 0:
				if (currentEntry == "software-update"):
					...
					if backupAvailable == 1:
						...
						self.session.open(MessageBox, _("Sorry, no settings backups found!"), MessageBox.TYPE_INFO, timeout = 10)
