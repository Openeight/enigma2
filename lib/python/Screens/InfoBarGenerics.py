...
from enigma import eTimer, eServiceCenter, eDVBServicePMTHandler, iServiceInformation, \
	iPlayableService, eServiceReference, eEPGCache, eActionMap, eDVBVolumecontrol
		...
		if self.ENABLE_RESUME_SUPPORT:
			...
			if (last > 900000) and (not length[1]  or (last < length[1] - 900000)):
				...
				if config.usage.on_movie_start.value == "ask" or not length[1]:
					Notifications.AddNotificationWithCallback(self.playLastCB, MessageBox, _("Do you want to resume this playback?") + "\n" + (_("Resume position at %s") % ("%d:%02d:%02d" % (l/3600, l%3600/60, l%60))), timeout=False)
		...
		self["HDMIActions"] = HelpableActionMap(self, "InfobarHDMIActions",
			{
				...
				"HDMIinLong":(self.HDMIInLong, _("Switch to HDMI in PiP mode")),
			}, prio=2)
