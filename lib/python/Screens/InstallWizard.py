...
class InstallWizard(Screen, ConfigListScreen):
	...
	def run(self):
		if self.index == self.STATE_UPDATE and config.misc.installwizard.hasnetwork.value:
			...
			if self.enabled.value:
				if self.channellist_type.value == "openeight":
					self.session.open(InstallWizardOpkgUpdater, self.index, _('Please wait (downloading channel list)'), OpkgComponent.CMD_REMOVE, {'package': 'enigma2-plugin-settings-henksat-' + self.channellist_type.value})
				else:
