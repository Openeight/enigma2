...
class PluginBrowser2(Screen):
	def __init__(self, session):
		Screen.__init__(self, session)
                self.skinName = "PluginBrowser2"
		self.firsttime = True
                print "Here in PluginBrowser2"
	...
	def updateListX(self):
		self.pluginlist = plugins.getPlugins(PluginDescriptor.WHERE_PLUGINMENU)
		self.list = [PluginEntryComponent(plugin) for plugin in self.pluginlist]
		self["list"].l.setList(self.list)

        def updateList(self):
                print "Here in updateList"
                xurl ="http://www.et-view-support.com/addons/XTA-team/Plugins/plugnames.txt"
...
#	        try:
        	print "html = ", html 
                self.pluginlist = []
                self.templist = plugins.getPlugins(PluginDescriptor.WHERE_PLUGINMENU)
                for plugin in self.templist:
                        name = str(plugin.name)
                        print "str(plugin.name) =", name
                        if name in html:
